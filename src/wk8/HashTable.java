...
public class HashTable<E> implements Set<E> {
    ...
    private static double loadFactorThreshold = 0.75;
    ...
    public boolean add(E element) {
        ...
        if(!table[index].contains(element)) {
            added = table[index].add(element);
            size++;
        }
        return added;
    }

    private void validateLoadFactor2() {
        HashTable<E> newHashTable = new HashTable<>((int)(table.length*1.5));
    ...
    private void validateLoadFactor() {
        if((double)(size+1)/table.length<loadFactorThreshold) {
            return;
        }

        List[] newTable = new List[(int)(table.length*1.5)];
        for(List<E> bucket : table) {
            if(bucket!=null) {
                for(E element : bucket) {
    ...
    public boolean contains(Object target) {
        List<E> bucket = getBucket(target);
        return bucket!=null && bucket.contains(target);
    }

    private List<E> getBucket(Object target) {
    ...
    public boolean remove(Object target) {
        List<E> bucket = getBucket(target);
        boolean removed = bucket!=null && bucket.remove(target);
        if(removed) {
            size--;
        }
        return removed;
